...
鲑鱼跑简称必须先还原到具体实体，再翻译动作或机制；不要把简称译成更宽泛的上位词。尤其是 `カタパ` 必须按 `カタパッド` 处理，中文用 `垫肩飞鱼`，不能泛化为 `飞鱼`。`Aのワンパン` 这类结构先锁定 `A` 是谁，再按语境译为一发秒、一击处理、金鲑鱼卵一击等。
...
只有用户明确要求解释、校对、比较版本、术语分析，或原文存在真实歧义/来源冲突且会影响译文时，才在译文后补充一句简短说明。不要为了展示查到的术语而解释。

长公告、文章、访谈、字幕稿也默认只输出译文正文，并保留原文结构。除非用户明确要求“解释”“校对”“列术语”或“给多个版本”，否则不要添加额外栏目。
